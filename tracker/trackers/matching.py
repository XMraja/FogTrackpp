...
def igda_distance(tracks, detections):
    """
    极速且抗量纲爆炸的 IGDA 矩阵 (完美映射版)
    """
    ...
    mu_p = np.zeros((N, 4), dtype=np.float32)
    sig_p = np.zeros((N, 4), dtype=np.float32)
    for i, track in enumerate(tracks):
        kf = track.kalman_filter.kf
        mu_p[i] = kf.x[:4].squeeze() 
        sig_p[i] = np.diag(kf.P)[:4] + epsilon 
        
    mu_q = np.zeros((M, 4), dtype=np.float32)
    sig_q = np.zeros((M, 4), dtype=np.float32)
    for j, det in enumerate(detections):
        ...
        mu_q[j] = np.array([cx, cy, w, h])
        
        # 把检测框的宽容度修正为 0.1 (恢复精确追踪的本色)
        std = np.array([0.1 * w, 0.1 * h, 0.1 * w, 0.1 * h]) 
        sig_q[j] = np.square(std) + epsilon

    diff_sq = (mu_p[:, None, :] - mu_q[None, :, :]) ** 2  
    sig_inv_sum = (1.0 / sig_q[None, :, :]) + (1.0 / sig_p[:, None, :]) 
    
    mean_diff_term = np.sum(diff_sq * sig_inv_sum, axis=2) 
    trace_term = np.sum(
        (sig_p[:, None, :] / sig_q[None, :, :]) + 
        (sig_q[None, :, :] / sig_p[:, None, :]), 
        axis=2
    ) - 8.0 
    
    jeffreys_div = 0.5 * (trace_term + mean_diff_term)
    
    # 将温度退火降至 20.0，这是兼顾高维度量和平滑梯度的最优解
    temperature = 20.0 
    cost_matrix = 1.0 - np.exp(-jeffreys_div / temperature)
