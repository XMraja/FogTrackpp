...
class LVTR(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        ...
        self.gamma_clear = nn.Parameter(torch.ones(1, c1, 1, 1))
        self.beta_clear = nn.Parameter(torch.zeros(1, c1, 1, 1))

        self.delta_gamma = nn.Parameter(torch.randn(1, c1, 1, 1) * 0.01)
        self.delta_beta = nn.Parameter(torch.randn(1, c1, 1, 1) * 0.01)

    def forward(self, x):
        alpha = self.alpha_extractor(x)
        
        dynamic_gamma = self.gamma_clear + alpha * self.delta_gamma
        dynamic_beta = self.beta_clear + alpha * self.delta_beta

        self.latest_alpha = alpha.detach().cpu().mean().item()
        
        return x * dynamic_gamma + dynamic_beta


class VL_Projector(nn.Module):
    ...
    def __init__(self, in_channels, clip_dim=512):
        ...
        self.proj = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(in_channels // 2, clip_dim, kernel_size=1, bias=False)
        )
