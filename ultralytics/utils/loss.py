...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.model = model  
        self.student_p5 = None

        self.model.model[-1].register_forward_pre_hook(self._intercept_student_hook)

        import clip  
        from ultralytics.nn.modules.block import VL_Projector

        clip_weight = "./CLIP/clip_model/ViT-B-32.pt"

        self.clip_model, _ = clip.load(clip_weight, device=device)
        ...
        self.vl_projector = VL_Projector(in_channels=256, clip_dim=512).to(device)
    ...
    def _intercept_student_hook(self, m, args):
        self.student_p5 = args[0][2]
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if "feat_teacher_p5" in batch:
            import torch.nn.functional as F

            img_clean = batch["img_clean"].to(self.device).float() / 255.0
            
            f_stu = self.student_p5
            f_tch = batch["feat_teacher_p5"]
            p_tch = self.vl_projector(f_tch) 

            with torch.no_grad():
                img_clean_224 = F.interpolate(img_clean, size=(224, 224), mode='bicubic', align_corners=False)
                img_clean_224 = (img_clean_224 - self.clip_mean) / self.clip_std
                v_clip = self.clip_model.encode_image(img_clean_224).float()
                v_clip = F.normalize(v_clip, p=2, dim=-1) 
                
            p_tch_global = F.adaptive_avg_pool2d(p_tch, (1, 1)).view(p_tch.size(0), -1)
            p_tch_global = F.normalize(p_tch_global, p=2, dim=-1)
            loss_align = F.mse_loss(p_tch_global, v_clip)

            current_epoch = batch.get("current_epoch", 0)
            phase1_epochs = batch.get("phase1_epochs", 50)
            t_pos = batch["text_anchors"]["t_pos"].to(self.device)
            
            if current_epoch < phase1_epochs: 
                t_neg = batch["text_anchors"]["t_neg_spa"].to(self.device) 
                lambda_mimic, lambda_clarity = 0.5, 0.05  
            else:
                t_neg = batch["text_anchors"]["t_neg_tex"].to(self.device) 
                lambda_mimic, lambda_clarity = 0.1, 0.2   

            b, _, h, w = f_stu.shape
            roi_mask = torch.zeros((b, 1, h, w), device=self.device)
            batch_idx = batch["batch_idx"].long()
            bboxes_xywh = batch["bboxes"] 
            
            for i in range(len(bboxes_xywh)):
                img_id = batch_idx[i]
                bx, by, bw, bh = bboxes_xywh[i]
                x1 = max(0, int((bx - bw / 2) * w))
                y1 = max(0, int((by - bh / 2) * h))
                x2 = min(w, int(math.ceil((bx + bw / 2) * w)))
                y2 = min(h, int(math.ceil((by + bh / 2) * h)))
                x2 = max(x1 + 1, x2)
                y2 = max(y1 + 1, y2)
                if x1 < x2 and y1 < y2:
                    roi_mask[img_id, 0, y1:y2, x1:x2] = 1.0
                    
            mask_sum = roi_mask.sum() + 1e-5

            f_stu_norm = F.normalize(f_stu, p=2, dim=1)
            f_tch_norm = F.normalize(f_tch, p=2, dim=1)
            mimic_cos = (f_stu_norm * f_tch_norm).sum(dim=1, keepdim=True)
            loss_mimic = ((1.0 - mimic_cos) * roi_mask).sum() / mask_sum
            
            p_stu = self.vl_projector(f_stu) 
            delta_t = F.normalize(t_pos - t_neg, p=2, dim=-1) 
            delta_v = F.normalize(p_tch.detach() - p_stu, p=2, dim=1) 
            
            delta_t_spatial = delta_t.view(1, 512, 1, 1).expand_as(delta_v)
            cos_sim = (delta_v * delta_t_spatial).sum(dim=1, keepdim=True)
            loss_clarity = ((1.0 - cos_sim) * roi_mask).sum() / mask_sum

            total_aux_loss = (loss_align * 0.02) + (loss_mimic * lambda_mimic) + (loss_clarity * lambda_clarity)
            loss[0] += total_aux_loss 
            
            self.student_p5 = None

        elif batch.get("is_phase3", False) and self.student_p5 is not None and "feat_student_prev_p5" in batch:
            import torch.nn.functional as F
            f_stu_t = self.student_p5
            f_stu_prev = batch["feat_student_prev_p5"]
            
            p_t = self.vl_projector(f_stu_t)
            p_prev = self.vl_projector(f_stu_prev)
            
            t_pos = batch["text_anchors"]["t_pos"].to(self.device)

            if "t_neg_tem" in batch["text_anchors"]:
                t_neg_tem = batch["text_anchors"]["t_neg_tem"].to(self.device)
            else:
                t_neg_tem = batch["text_anchors"]["t_neg_spa"].to(self.device)
                
            delta_t = F.normalize(t_pos - t_neg_tem, p=2, dim=-1)
            ...
            mask_sum_spatial = roi_mask.sum(dim=(2, 3)) + 1e-5
            p_t_pooled = (p_t * roi_mask).sum(dim=(2, 3)) / mask_sum_spatial # (B, 512)
            p_prev_pooled = (p_prev * roi_mask).sum(dim=(2, 3)) / mask_sum_spatial # (B, 512)
            
            delta_v = p_t_pooled - p_prev_pooled # (B, 512)
            delta_v_norm = F.normalize(delta_v, p=2, dim=1)
            

            cos_sim = (delta_v_norm * delta_t).sum(dim=1) # (B,)

            valid_batch_mask = (roi_mask.sum(dim=(1, 2, 3)) > 0).float() # Shape: (B,)

            loss_temp_dir = ((1.0 - cos_sim) * valid_batch_mask).sum() / (valid_batch_mask.sum() + 1e-5)

            loss[0] += loss_temp_dir * 0.1
