...
class v8DetectionLoss:
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if "feat_teacher_p5" in batch:
            ...
            residual = torch.mean(torch.abs(img_foggy - img_clean), dim=1, keepdim=True)
            gt_visibility = torch.exp(-5.0 * residual) 
            loss_dvp = 0.0
            dvp_count = 0
            for m in self.model.modules():
                if hasattr(m, 'alpha_map') and m.alpha_map is not None:
                    pred_alpha = m.alpha_map
                    _, _, h, w = pred_alpha.shape
                    gt_vis_down = F.interpolate(gt_visibility, size=(h, w), mode='area')
                    loss_dvp += F.mse_loss(pred_alpha, gt_vis_down)
                    dvp_count += 1
                    m.alpha_map = None
            if dvp_count > 0: loss_dvp /= dvp_count
            ...
            with torch.no_grad():
                img_clean_224 = F.interpolate(img_clean, size=(224, 224), mode='bicubic')
            ...
            loss_mimic_global = (1.0 - mimic_cos).mean()

            loss_mimic_roi = ((1.0 - mimic_cos) * roi_mask).sum() / mask_sum
            loss_mimic = loss_mimic_global * 0.2 + loss_mimic_roi * 1.0
            ...
            loss_clarity_global = (1.0 - cos_sim).mean()
            loss_clarity_roi = ((1.0 - cos_sim) * roi_mask).sum() / mask_sum
            loss_clarity = loss_clarity_global * 0.1 + loss_clarity_roi * 1.0

            total_aux_loss = (loss_dvp * 0.1) + (loss_align * 0.05) + (loss_mimic * lambda_mimic) + (loss_clarity * lambda_clarity)
