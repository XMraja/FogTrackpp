...
class YOLODataset(BaseDataset):
    ...
    def load_image(self, i):
        """加载图片，并将雾天图(前3通道)和清晰图(后3通道)合并为 6 通道图片返回"""
        im, (h, w), (h0, w0) = super().load_image(i)

        im_path = self.im_files[i]
  
        clean_path = str(im_path).replace('fogmvtpp/inhomogeneous-fog', 'vesselmot')
        ...
        if os.path.exists(clean_path):
            clean_im = cv2.imread(clean_path)
            if clean_im is None:
                clean_im = im.copy()
            elif clean_im.shape[:2] != im.shape[:2]:
                clean_im = cv2.resize(clean_im, (im.shape[1], im.shape[0]))
        else:
            clean_im = im.copy()

        im_6c = np.concatenate([im, clean_im], axis=-1)
        
        return im_6c, (h, w), (h0, w0)
    ...
    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        ...
        # import torch
        try:
            self.text_anchors = torch.load('./weights/text_anchors.pt')
        except FileNotFoundError:
            self.text_anchors = None
            print("⚠️ 未找到 text_anchors.pt，训练前请确保已生成该文件！")
        # ==========================================================
    ...
    def get_image_and_label(self, index):
        # 先调用隐身的父类 BaseDataset 获取原版的所有数据 (包含雾图和标签)
        label = super().get_image_and_label(index)
        
        # 利用路径替换规则，将 /fog/images/ 替换为 /clean/images/
        im_file_clean = label["im_file"].replace('/fog/images/', '/clean/images/')
        
        img_clean = cv2.imread(im_file_clean)
        if img_clean is None:
            # 兜底保护：防止路径错位导致程序崩溃
            img_clean = label["img"].copy()
            
        label["img_clean"] = img_clean
        label["text_anchors"] = self.text_anchors
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            value = values[i]
            if k in {"img", "img_clean", "text_feats", "sem_masks"}:
