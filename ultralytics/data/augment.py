...
class LetterBox:
    ...
    def __call__(self, labels: dict[str, Any] | None = None, image: np.ndarray = None) -> dict[str, Any] | np.ndarray:
        ...
        if len(labels):
            ...
            # 👇👇👇 【请在这里加上这段 LookTrack 新增 5.1】 👇👇👇
            if "img_prev" in labels:
                img_p = labels["img_prev"]
                img_p = cv2.resize(img_p, new_unpad, interpolation=self.interpolation)
                if c == 3:
                    img_p = cv2.copyMakeBorder(
                        img_p, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(self.padding_value,) * 3
                    )
                labels["img_prev"] = img_p
...
class Format:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if "img_clean" in labels:
            img_c = labels.pop("img_clean")
            if len(img_c.shape) < 3:
                img_c = img_c[..., None]
            # HWC to CHW -> contiguous, 并且进行 BGR to RGB 切片 ([::-1])
            img_c = np.ascontiguousarray(img_c.transpose((2, 0, 1))[::-1])
            labels["img_clean"] = torch.from_numpy(img_c)

        # 👇👇👇 【请在这里加上这段 LookTrack 新增 5.2】 👇👇👇
        if "img_prev" in labels:
            img_p = labels.pop("img_prev")
            if len(img_p.shape) < 3:
                img_p = img_p[..., None]
            img_p = np.ascontiguousarray(img_p.transpose((2, 0, 1))[::-1])
            labels["img_prev"] = torch.from_numpy(img_p)
        # 👆👆👆 ========================================= 👆👆👆
